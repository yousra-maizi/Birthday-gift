...
// Dynamically import both .png and .jpg images
const imageFiles = import.meta.glob("../assets/music/*.{png,jpg}");
    ...
    const loadImages = async () => {
      const loadedImages = await Promise.all(
        config.musicGallery.map(async (song, index) => {
          // Construct both PNG and JPG paths
          const pngPath = `../assets/music/${index + 1}.png`;
          const jpgPath = `../assets/music/${index + 1}.jpg`;

          let imageModule = null;

          // Check which file exists
          if (imageFiles[pngPath]) {
            imageModule = await imageFiles[pngPath]();
          } else if (imageFiles[jpgPath]) {
            imageModule = await imageFiles[jpgPath]();
          }

          // If image exists, return song details
          if (imageModule) {
            return {
          ...
          return null; // Skip if no image found
        })
      );

      setSongs(loadedImages.filter((song) => song !== null)); // Remove null entries
    };
