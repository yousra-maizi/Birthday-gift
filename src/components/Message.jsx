...
const imageFiles = import.meta.glob('../assets/MessageImage*.{png,jpg,jpeg}');
      ...
      const loadedImages = await Promise.all(
        Object.keys(imageFiles)
          .sort((a, b) => {
            ...
            const aNum = parseInt(a.match(/MessageImage(\d+)\.(png|jpg|jpeg)/)?.[1] || 0, 10);
            const bNum = parseInt(b.match(/MessageImage(\d+)\.(png|jpg|jpeg)/)?.[1] || 0, 10);
          ...
          .map(async (path, index) => {
            ...
            return {
             ...
             // title desactivated title: config.messageGallery[index]?.title || 'No Title',
